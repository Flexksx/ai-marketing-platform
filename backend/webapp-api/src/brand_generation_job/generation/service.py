...
async def run_brand_generation_job(
    ...
    try:
        job = await brand_generation_job_service.get(job_id)

        if job.result:
            await _update_job(job_id, JobStatus.IN_PROGRESS, job.result)
        logger.info(
        ...
        job = await _update_job(job_id, JobStatus.IN_PROGRESS, scraping_result)
        logger.info(
        ...
        job = await _update_job(job_id, JobStatus.COMPLETED, data_extraction_result)
        logger.info(
