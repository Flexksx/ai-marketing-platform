...
_HEAD_METHOD_NOT_ALLOWED_CODES = (403, 405, 501)
_URL_VALIDATION_MAX_RETRIES = 5
_URL_VALIDATION_DELAY_SECONDS = 2
...
async def scrape_brand(
    ...
    try:
        scrape_result = await scraper.scrape(job.website_url)
        logger.info(
            f"Scraper completed for job {job.id}: "
            f"{len(scrape_result.text)} chars, {len(scrape_result.image_urls)} images, "
            f"{len(scrape_result.video_urls)} videos, {len(scrape_result.page_urls)} pages",
            extra={"job_id": job.id},
        )
        ...
        if scrape_result.screenshot:
            ...
            is_accessible = await _validate_url_accessible(
        ...
        about_url = _find_about_url(job.website_url, scrape_result.page_urls)
        if about_url:
            logger.info(
            ...
            try:
                about_scrape_result = await scraper.scrape(about_url)
                logger.info(
                    f"About page scraped successfully for job {job.id}: "
                    f"{len(about_scrape_result.text)} chars",
                    extra={"job_id": job.id},
                )
                scrape_result = _merge_scrape_results(
...
async def _validate_url_accessible(
    url: str,
    job_id: str,
    max_retries: int = _URL_VALIDATION_MAX_RETRIES,
    delay_seconds: int = _URL_VALIDATION_DELAY_SECONDS,
) -> bool:
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if response.is_success:
                    ...
                    return True
                if response.status_code in _HEAD_METHOD_NOT_ALLOWED_CODES:
                    logger.info(
...
def _find_about_url(base_url: str, page_urls: list[str]) -> str | None:
...
def _merge_scrape_results(main: ScrapeResult, about: ScrapeResult) -> ScrapeResult:
