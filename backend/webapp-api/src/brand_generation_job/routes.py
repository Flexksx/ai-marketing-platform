...
async def start(
    ...
    create_request = BrandGenerationJobCreateRequest(website_url=request.website_url)
    return await brand_generation_job_service.start(
        user_id, create_request, scraper, background_tasks
    )


@router.get("/{job_id}", response_model=BrandGenerationJobResponse)
async def get(job_id: str = Depends(validate_brand_generation_job_access)):
    return await brand_generation_job_service.get(job_id)
...
async def accept(
    ...
    return await brand_generation_job_service.accept(job_id, request)
