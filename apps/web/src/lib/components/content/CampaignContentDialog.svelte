...
<Dialog.Root open={isOpen} onOpenChange={(newOpen) => !newOpen && handleClose()}>
	<Dialog.Overlay class="bg-black/10 backdrop-blur-sm" />
	<Dialog.Content class="p-0 border-0 bg-transparent shadow-none" showCloseButton={false}>
		<div class="absolute top-4 -right-38 flex flex-col gap-2 z-10 items-start max-sm:hidden">
		...
		<div class="max-h-[90vh] overflow-y-auto">
			<div class="sm:hidden flex flex-row gap-2 justify-center mb-2">
				...
				<Button variant="secondary" onclick={handleClose} class={buttonStyle}>
					<X class="h-4 w-4" />
				</Button>
			</div>
